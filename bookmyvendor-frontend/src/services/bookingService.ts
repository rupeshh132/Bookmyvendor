...
export const bookingService = {
  ...
  rejectRequest: async (bookingId: string): Promise<BookingRequestDto> => {
    const res = await api.put(`/api/v1/bookings/${bookingId}/reject`)
    return res.data.data
  },

  // Customer: Cancel Request
  cancelRequest: async (bookingId: string): Promise<BookingRequestDto> => {
    const res = await api.put(`/api/v1/bookings/${bookingId}/cancel`)
    return res.data.data
  },
